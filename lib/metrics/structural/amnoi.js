      ...
      declarations.forEach((declaration) => {
        if (Node.isFunctionDeclaration(declaration)) {
          if (declaration.isImplementation() && hasOverload(declaration)) {
            ...
            produceResult(module, name, declaration.getKind(), index, declaration.getOverloads().map(overload => overload.getStructure()));
          }
          ...
          declaration.getInstanceMethods().concat(declaration.getStaticMethods())
            .forEach(method => {
              if (isPublic(method) && method.isImplementation() && hasOverload(method)) {
                ...
                produceResult(module, name, declaration.getKind(), index, method.getOverloads().map(overload => overload.getStructure()));
              }
            });
