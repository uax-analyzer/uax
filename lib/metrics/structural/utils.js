...
const normalizeType = (declaration, type) => {
  ...
  const regex2 = new RegExp(`(?<![a-zA-Z0-9_$]+)(${[...constraint.keys()].join("|")})(?![a-zA-Z0-9_$]+)`, 'g');
  for (let [key, value] of constraint) {
    const matches = value.matchAll(regex2);
...
const setModuleIfNotSet = (results, modulePath) => {
  if (!results.hasOwnProperty(modulePath)) {
    results[modulePath] = {}
  }
}

// enum to distinguish different types of results
