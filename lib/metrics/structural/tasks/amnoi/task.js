...
function typeSet(modulePath, types, path) {
  let sourceFile = project.getSourceFile(modulePath);
  ...
  for (let i = 0; i < types.length; i++) {
    for (let j = 0; j < types.length; j++) {
      ...
      if (a !== 'any' && b !== 'any') {
        ...
        //console.log(tmpSourceFile.getLineAndColumnAtPos(endPos).line);
      }
  ...
  //return equalTypes.concat([...equalityChecking.values()]).map(equalTypesIndexes => types[equalTypesIndexes.index1]);
