...
const isModuleSet = (results, modulePath) => {
  if (!results.hasOwnProperty(modulePath)) {
    results[modulePath] = {}
  }
}

const produceFunctionResult = results => (modulePath, name, kind, analysisResults) => {
  isModuleSet(results, modulePath);
...
const produceMethodResult = results => (modulePath, name, kind, methodName, analysisResults) => {
  isModuleSet(results, modulePath);
  const moduleResult = results[modulePath];
...
const prepareResult = results => {
  return [produceFunctionResult(results), produceMethodResult(results)];
}
