...
export default async function computeAPLCI() {
          ...
          case StructureKind.Function:
            if (declaration.overloads.length > 0) {
              declaration.overloads.forEach(overload => {
                overload.parameters.map(param => param.name).forEach(name => parametersName.add(name));
                overload.functionName = name;
                overload.modulePath = modulePath;
                functionsDeclarations.push(overload);
              });
            } else {
              declaration.parameters.map(param => param.name).forEach(name => parametersName.add(name));
              ...
              .forEach(method => {
                if (method.overloads.length > 0) {
                  method.overloads.forEach(methodOverload => {
                    methodOverload.parameters.map(param => param.name).forEach(name => parametersName.add(name));
                  ...
                  method.parameters.map(param => param.name).forEach(name => parametersName.add(name));
  ...
  for (let [_, decs] of mapping) {
    if (decs[0].length + decs[1].length >= 3) {
      greaterThan2++;
      if (decs[0].length === 0 || decs[1].length === 0) {
        // the cardinality of the set must be at least 3 and one of the two subsets must be empty
        consistent++;
      }
    }
  }
  ...
  return {
    ...
    metricName: 'APLCI',
    usability: {
      score: safeDiv(consistent, greaterThan2)
    }
  };
}